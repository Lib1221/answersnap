...
<template>
  ...
  >
    ...
    <p class="text-[13px] text-graphite-2">
      ...
      {{ t('resume_needs_job', 'above, then get your resume bullets reworded for this job.') }}
    </p>
  </section>
  ...
  <template v-else>
    <section class="card flex flex-col gap-3 p-4" data-testid="resume">
      <div class="flex items-center gap-2.5">
        ...
        <div class="min-w-0 flex-1">
          <h2 class="text-[15px] leading-tight font-[650]">
            {{ t('resume_title', 'Tailor my resume') }}
          </h2>
          <p class="truncate text-[12.5px] text-graphite-2">
            {{ t('resume_for', 'For $1', props.job.label(props.job.job.value)) }}
          </p>
        </div>
      </div>
      ...
      >
        ...
        {{ t('resume_running', 'Tailoring your resume…') }}
      </p>
      ...
      >
        ...
        {{ res ? t('resume_run_again', 'Tailor again') : t('resume_title', 'Tailor my resume') }}
      </button>
    </section>
    ...
    <template v-if="res && r.status.value !== 'running'">
      <section class="card flex flex-col gap-2 p-4">
        <div class="flex items-center justify-between">
          <p class="eyebrow">{{ t('resume_summary', 'Summary') }}</p>
          <button
          ...
          >
            ...
            {{ copied === 'summary' ? t('resume_copied', 'Copied') : t('resume_copy', 'Copy') }}
          </button>
        </div>
      ...
      <section class="card flex flex-col gap-2 p-4">
        <div class="flex items-center justify-between">
          <p class="eyebrow">{{ t('resume_bullets', 'Bullets') }}</p>
          <button
            class="btn btn-quiet min-h-0 text-[12.5px]"
            type="button"
          ...
          >
            ...
            {{ copied === 'all' ? t('resume_copied', 'Copied') : t('resume_copy_all', 'Copy all') }}
          </button>
        </div>
          ...
          <li v-for="(b, i) in res.bullets" :key="i" class="flex flex-col gap-1">
            <p class="text-[13.5px]">
              ...
              <span
                ...
                :title="t('resume_assumed_title', 'Not in your data: check before using')"
                >{{ t('resume_assumed', 'Assumed') }}</span
              >
            </p>
            <p v-if="b.original" class="text-[12px] text-graphite-2">
              {{ t('resume_was', 'Was: $1', b.original) }}
            </p>
            <p v-else class="text-[12px] text-graphite-2">
              {{ t('resume_new_bullet', 'New bullet for a gap in your profile.') }}
            </p>
            <button
            ...
            >
              ...
              {{ copied === `b${i}` ? t('resume_copied', 'Copied') : t('resume_copy', 'Copy') }}
            </button>
          </li>
        </ul>
      </section>
      ...
      <section v-if="res.skills.length" class="card flex flex-col gap-2 p-4">
        <div class="flex items-center justify-between">
          <p class="eyebrow">{{ t('resume_skills', 'Skills, in this order') }}</p>
          <button
            class="btn btn-quiet min-h-0 text-[12.5px]"
            type="button"
          ...
          >
            ...
            {{ copied === 'skills' ? t('resume_copied', 'Copied') : t('resume_copy', 'Copy') }}
          </button>
        </div>
